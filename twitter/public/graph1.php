<?php 
	...
	if(isset($_GET['hash'])){
		...
		if(isset($_GET['year'])&&!isset($_GET['month'])){
				...
				$goto="<a href=\"".$_SERVER['PHP_SELF']."?hash=".urlencode($hash)."\" class=\"graph\">Go Back</a>";
			...
			while($row=mysql_fetch_array($date_day)){
					...
					$y=$row['dates'];
					$year_month=$year.",".$y;
					$yearname="<a  href=\"".$_SERVER['PHP_SELF']."?hash=".urlencode($hash)."&year=".$year."&month=".$y."\">".$y."</a>";
				...
				while($row1=mysql_fetch_array($date_user)){
					...
					$topuser.= "<b><a  style=\"color:red\" href=\"hashtag.php?hash=".urlencode($hash)."&user=".$row1['users']."\">".$row1['users']."</a>:</b> ".$row1['count']." tweets<br/>";
					$i++;
				...
				$goto="<a href=\"".$_SERVER['PHP_SELF']."?hash=".urlencode($hash)."&year=".$year."\" class=\"graph\">Go Back</a>";
				$index=$month.",".$year;
			...
			while($row=mysql_fetch_array($date_day)){
					...
					$y=$row['dates'];
					$yearname="<a  href=\"". $_SERVER['PHP_SELF']."?hash=".urlencode($hash)."&year=".$y."\">".$y."</a>";
					$data['year'][]=$yearname;
...
<html>
	...
	<body>
			...
			<div id="container" style="width: 645px; height:200px;"></div>
			</div>
			</div>
	</body>
</html>