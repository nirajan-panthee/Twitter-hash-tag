<html>
	...
	<body>
		<?php
			...
			<caption><?php echo "Graph OF ".$month." ".$year." OF ".$hash; ?></caption>
				<thead>
					<tr><?php
						foreach($day as $y){?>
			
							<th><?php echo $y['0']; ?></th><?php
			...
			$date_day=mysql_query("SELECT DISTINCT(UserID) as users,COUNT(Tweets) as count FROM `infotweets` WHERE HashTag='$hash' AND FROM_UNIXTIME(DATETIME,'%Y,%b')='$year_month' GROUP BY users ORDER BY count DESC LIMIT 0,3 ");
