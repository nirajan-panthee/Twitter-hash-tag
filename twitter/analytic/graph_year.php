<html>
	<head>
		...
		<script type="text/javascript">
			$(function(){
				$('#year').visualize({type: 'line', width: '380px'});
	...
	<body>
		...
		<div style="width:500px;overflow:hidden;margin:0px;float:left">
			<table id="year" style="display:none">
				<caption><?php echo $hash; ?></caption>
				<thead>
					<tr><?php
						foreach($year as $y){?>
			
							<th><a href="graph_month.php?year=<?php echo $y['0']; ?>&hash=<?php echo urlencode($hash); ?>"><?php echo $y['0']; ?></a></th><?php
		...
		<div style="float:left;background-color:#F2F2F2;border-radius:10px;padding:10px;width:400px">
			<h2 align="center">Top Users</h2><?php
		
			$date_day=mysql_query("SELECT DISTINCT(UserID) as users,COUNT(Tweets) as count FROM `infotweets` WHERE HashTag='$hash' GROUP BY users ORDER BY count DESC LIMIT 0,3 ");
